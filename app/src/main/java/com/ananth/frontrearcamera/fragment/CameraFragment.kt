...
//        backgroundHandler?.post(ImageSaver(it.acquireNextImage(), file))
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mRecordButton = view.findViewById(R.id.record_video)
        mRecordButton.setOnClickListener { view ->
            if(mIsRecording) {
                mIsRecording = false
                mRecordButton.text = "Recording..."
                mMediaRecorder?.stop()
    ...
    override fun onRequestPermissionsResult(
        ...
        if(requestCode == REQUEST_WRITE_EXTERNAL_STORAGE_PERMISSION_RESULT) {
            if(grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                if(mIsRecording) {
                    mIsRecording = true
                    Toast.makeText(context, "Storage Permissions granted successfully", Toast.LENGTH_SHORT).show()
                }
                else{
                    Toast.makeText(context, "App needs to save video to run", Toast.LENGTH_SHORT).show()
...
//                        activity.showToast("Failed")
    ...
    companion object {
         ...
         */
        private val REQUEST_WRITE_EXTERNAL_STORAGE_PERMISSION_RESULT: Int = 1
    ...
    //Rear camera TextureView recording only as of now
...
//        mMediaRecorder?.setAudioSource(MediaRecorder.AudioSource.MIC)
        ...
        mMediaRecorder?.setVideoSize(previewSizeRear.width, previewSizeRear.height)
        mMediaRecorder?.setVideoEncoder(MediaRecorder.VideoEncoder.H264)
//        mMediaRecorder?.setAudioEncoder(MediaRecorder.AudioEncoder.AAC)
        mMediaRecorder?.setOrientationHint(rotation)
        mMediaRecorder?.prepare()
        Log.d(TAG, "setupMediaRecorder: " + "Inside media recorder")
    }
    ...
    private fun createVideoFileName(): File? {
        ...
        val videoFile: File = File.createTempFile(prepend, ".mp4", mVideoFolder)
    ...
    private fun startRecord() {
        try {
            ...
            surfaceTexture.setDefaultBufferSize(previewSizeRear.width, previewSizeRear.height)
            val previewSurface = Surface(surfaceTexture)
            val recordSurface = mMediaRecorder?.surface
            previewRequestBuilderRear =
                cameraDeviceRear?.createCaptureRequest(CameraDevice.TEMPLATE_RECORD)!!
            previewRequestBuilderRear.addTarget(previewSurface)
            if (recordSurface != null) {
                previewRequestBuilderRear.addTarget(recordSurface)
            }
            cameraDeviceRear?.createCaptureSession(
                listOf(
                    previewSurface,
                    recordSurface,
                    imageReaderRear?.surface
                ),
                object : CameraCaptureSession.StateCallback() {
                    override fun onConfigured(session: CameraCaptureSession) {
                        captureSessionRear = session
                        try {
                            captureSessionRear!!.setRepeatingRequest(
                                previewRequestBuilderRear.build(), null, null
                            )
                        } catch (e: CameraAccessException) {
                            e.printStackTrace()
                        }
        ...
        Log.d(TAG, "startRecord: " + "Inside startRecord")
    }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (activity?.let {
                ...
                if (shouldShowRequestPermissionRationale(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                    Toast.makeText(context, "app needs to be able to save videos", Toast.LENGTH_SHORT)
                        .show()
                }
                requestPermissions(
...
//            val stillCaptureCallback: CameraCaptureSession.CaptureCallback = object : CameraCaptureSession.CaptureCallback() {
